...
if [ "$node_actual" != "v$node_expected" ] || [ "$node_actual_platform" != "$node_platform" ] || [ "$node_actual_arch" != "$node_arch" ] || \
  ...
  printf 'TOOLCHAIN_MISMATCH: expected node=v%s pnpm=%s go=%s runtime=%s nodeArch=%s goArch=%s; actual node=%s pnpm=%s go=%s nodeRuntime=%s/%s goRuntime=%s/%s\n' \
    "$node_expected" "$pnpm_expected" "$go_expected" "$platform" "$node_arch" "$go_arch" \
    "${node_actual:-missing}" "${pnpm_actual:-missing}" "${go_actual:-missing}" \
